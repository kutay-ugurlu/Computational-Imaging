...
%% b
for ratio = [3 4]
    variance = var(y)/ratio; % SNR adjustment
...
%% c: Inverse Filtering in Frequency domain 
figure
size_Y = size(Y_blurred);
X_inv = ifft2(fft2(Y_blurred,size_Y(1),size_Y(2))./fft2(F,size_Y(1),size_Y(2)),size_Y(1),size_Y(2));
X_inv_noisy = ifft2(fft2(y_img)./fft2(F,size_Y(1),size_Y(2)),size_Y(1),size_Y(2));
subplot(1,2,1)
...
for ratio = 1:10
    ...
    subplot(4,3,ratio+2)
    imagesc(X_back)
    title({['Ratio:',num2str(ratio)],['Min Eigenvalue index: ',num2str(index)]})
end
